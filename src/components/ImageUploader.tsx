    ...
    <div className="space-y-3">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-colors ${
          dragOver
            ? 'border-mink-400 bg-mink-50'
            : 'border-stone-300 hover:border-mink-300 hover:bg-ivory-50'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-2">
            <Loader2 size={28} className="text-mink-500 animate-spin" />
            <p className="text-sm text-ink-500">Uploading...</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">
            <div className="w-12 h-12 rounded-full bg-ivory-200 flex items-center justify-center">
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
          {images.map((url, i) => (
            <div
              key={i}
              className="relative group aspect-square rounded-2xl overflow-hidden bg-ivory-100 border border-stone-300"
            >
              <img
                src={url}
              ...
              />
              {i === 0 && (
                <span className="absolute top-1 left-1 bg-ink-700 text-ivory-50 text-[10px] font-medium px-1.5 py-0.5 rounded">
                  Cover
                </span>
              )}
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                <button
                  ...
                  disabled={i === 0}
                  className="w-7 h-7 rounded-full bg-white/90 text-ink-700 flex items-center justify-center text-xs disabled:opacity-30 disabled:cursor-not-allowed hover:bg-white"

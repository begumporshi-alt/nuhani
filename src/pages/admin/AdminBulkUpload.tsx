  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-serif text-ink-800">Bulk Upload</h1>
          <p className="text-ink-400 text-sm mt-1">Upload multiple product images at once. Draft products are created — edit them later to add details.</p>
        </div>
      </div>
      ...
      <div
        className={`border-2 border-dashed rounded-2xl p-12 text-center transition-all cursor-pointer ${
          dragOver ? 'border-mink-400 bg-mink-50' : 'border-stone-300 hover:border-mink-300 bg-white'
        }`}
      ...
      >
        ...
        />
        <Upload size={40} className="text-ink-300 mx-auto mb-4" />
        <p className="text-ink-600 font-medium mb-1">Drop images here or click to browse</p>
        <p className="text-sm text-ink-400">PNG, JPG, WebP — each image becomes a separate draft product</p>
      </div>
      ...
      {images.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-serif text-ink-800">{images.length} images selected</h2>
            <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {images.map((img, i) => (
              <div key={i} className="relative group">
                <div className="aspect-square rounded-2xl overflow-hidden bg-ivory-100 relative">
                  <img src={img.url} alt={img.name} className="w-full h-full object-cover" />
                  {img.uploading && (
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <div className="w-6 h-6 border-2 border-ivory-50 border-t-transparent rounded-full animate-spin" />
                    </div>
                  )}
                  {img.uploaded && (
                ...
                <p className="text-xs text-ink-500 mt-1 truncate">{img.name}</p>
                <button
      ...
      {drafts.length > 0 && (
        <div className="mt-8">
          <h2 className="text-lg font-serif text-ink-800 mb-4">Draft Products Created</h2>
          <div className="card overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-ivory-100">
                <tr>
                  <th className="text-left py-3 px-4 text-ink-700">Image</th>
                  <th className="text-left py-3 px-4 text-ink-700">Product Name</th>
                  <th className="text-left py-3 px-4 text-ink-700">Slug</th>
                  <th className="text-left py-3 px-4 text-ink-700">Status</th>
                  <th className="text-right py-3 px-4 text-ink-700">Action</th>
                </tr>
              </thead>
              <tbody>
                {drafts.map((draft) => (
                  <tr key={draft.id} className="border-t border-stone-200">
                    <td className="py-3 px-4">
                      {draft.imageUrls[0] ? (
                        <img src={draft.imageUrls[0]} alt={draft.name} className="w-12 h-12 rounded-2xl object-cover" />
                      ) : (
                        <div className="w-12 h-12 rounded-2xl bg-ivory-100 flex items-center justify-center">
                          <ImageIcon size={18} className="text-ink-300" />
                        </div>
                      )}
                    </td>
                    <td className="py-3 px-4 font-medium text-ink-800">{draft.name}</td>
                    <td className="py-3 px-4 text-ink-500">{draft.slug}</td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 text-right">
                      <a
                        ...
                        className="text-mink-500 hover:text-mink-600 text-sm font-medium"
                      >
                        Edit Details
                      </a>
                    </td>
      ...
      {/* How it works */}
      <div className="mt-12 bg-mink-50 rounded-2xl p-6">
        <h3 className="font-serif text-ink-800 mb-3">How Bulk Upload Works</h3>

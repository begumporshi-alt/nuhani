...
export default function AdminChat() {
  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      <AdminBackLink />
      <h1 className="text-3xl font-serif text-ink-800 mb-8">Customer Messages</h1>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="space-y-2 p-4">
                {[...Array(4)].map((_, i) => <div key={i} className="h-16 bg-ivory-100 rounded-2xl animate-pulse" />)}
              </div>
            ) : filtered.length === 0 ? (
              <p className="text-center text-ink-400 py-12 text-sm">No conversations found.</p>
            ) : (
              filtered.map((conv) => (
                <button
                  key={conv.id}
                  onClick={() => setSelectedId(conv.id)}
                  className={`w-full text-left p-4 border-b border-ivory-100 hover:bg-ivory-50 transition-colors ${
                    selectedId === conv.id ? 'bg-mink-50' : ''
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      conv.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-ivory-200 text-ink-400'
                    }`}>
                      {conv.status}
                    </span>
                  </div>
                    ...
                    <div
                      className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-sm ${
                        msg.sender === 'user'
                          ...
                          : msg.sender === 'bot'
                          ? 'bg-ink-100 text-ink-700 rounded-br-sm'

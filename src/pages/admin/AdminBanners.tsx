  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-serif text-ink-800">Banners</h1>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(2)].map((_, i) => <div key={i} className="h-32 bg-ivory-100 rounded-2xl animate-pulse" />)}</div>
      ) : banners.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {banners.map((b) => (
            <div key={b.id} className="card overflow-hidden">
              <div className="relative h-40 bg-ivory-100">
                {b.image_url ? (
                  <img src={b.image_url} alt={b.title} className="w-full h-full object-cover" />
                ) : (
                  <div className="flex items-center justify-center h-full"><ImageIcon size={32} className="text-ink-300" /></div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                <div className="absolute bottom-3 left-4 right-4">
                  <h3 className="text-ivory-50 font-serif text-lg">{b.title}</h3>
                  {b.subtitle && <p className="text-ivory-100/80 text-sm">{b.subtitle}</p>}
                </div>
              ...
              <div className="p-4 flex items-center justify-between">
                <span className="text-xs text-ink-400">Order: {b.sort_order}</span>
                <div className="flex gap-2">
                  <button onClick={() => openEdit(b)} className="p-2 rounded-2xl hover:bg-ivory-200 text-ink-600"><Pencil size={16} /></button>
                  <button onClick={() => handleDelete(b.id)} className="p-2 rounded-2xl hover:bg-red-100 text-red-500"><Trash2 size={16} /></button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="text-center py-12">
          ...
          <p className="text-ink-400">No banners yet.</p>
        </div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          ...
          <div className="relative bg-ivory-50 rounded-2xl max-w-md w-full max-h-[85vh] overflow-y-auto p-6 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-serif text-ink-800">{editing ? 'Edit Banner' : 'Add Banner'}</h2>
              <button onClick={() => setShowForm(false)} className="text-ink-400"><X size={24} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-ink-600 mb-1.5 block">Title *</label>
                <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="input-field" />
              </div>
              <div>
                <label className="text-sm text-ink-600 mb-1.5 block">Subtitle</label>
              ...
              <div>
                <label className="text-sm text-ink-600 mb-1.5 block">Link URL</label>
              ...
              <div>
                <label className="text-sm text-ink-600 mb-1.5 block">Button Text</label>

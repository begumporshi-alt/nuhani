  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-serif text-ink-800">Review Moderation</h1>
          {pendingCount > 0 && (
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-ivory-100 rounded-2xl animate-pulse" />)}</div>
      ) : filtered.length > 0 ? (
            ...
            <div key={review.id} className="card p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          size={14}
                          className={i < review.rating ? 'fill-champagne-400 text-champagne-400' : 'text-stone-300'}
                        />
                      ))}
                    </div>
                    {review.is_approved ? (
                  ...
                  {review.body && <p className="text-sm text-ink-500 leading-relaxed mb-3">{review.body}</p>}
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  ...
                  {review.is_approved && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors text-sm font-medium"
                    >
                      <X size={16} /> Unapprove
                    </button>
                  )}

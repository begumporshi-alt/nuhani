...
export default function AdminCourier() {
  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      <AdminBackLink />
      <h1 className="text-3xl font-serif text-ink-800 mb-8">Courier Management</h1>

      <div className="card p-5 mb-6 bg-mink-50">
        <div className="flex items-start gap-3">
          <Truck className="text-ink-700 shrink-0 mt-0.5" size={20} />
          <div className="text-sm text-ink-600">
            <p className="font-medium text-ink-800 mb-1">Courier Integration</p>
      ...
      {loading ? (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-sm text-ink-400">{formatBDT(order.total_amount)}</span>
                    ...
                    {order.courier_tracking_id && (
                      <div className="mt-2 flex items-center gap-2 text-xs">
                        <span className="badge bg-ivory-200 text-ink-600">{order.courier_name ?? 'Courier'}</span>
                        <span className="text-ink-500">Tracking: {order.courier_tracking_id}</span>
                    ...
                    {!order.courier_tracking_id ? (
                      <>
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-mink-100 text-ink-700 hover:bg-mink-200 transition-colors text-sm font-medium"
                        >
                          <Send size={14} /> Auto Create
                        </button>
                        <button
                          onClick={() => { setSelectedOrder(order); setTrackingInput(''); setCourierName('') }}
                          className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-ivory-200 text-ink-700 hover:bg-ivory-300 transition-colors text-sm font-medium"
                        >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors text-sm font-medium"
                      >
        ...
        <div className="text-center py-16">
          <Truck size={48} className="text-stone-300 mx-auto mb-4" />
          <p className="text-ink-400">No orders ready for shipping.</p>
        </div>
      )}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelectedOrder(null)}>
          ...
          <div className="relative bg-ivory-50 rounded-2xl max-w-md w-full p-6 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-xl font-serif text-ink-800 mb-1">Add Tracking Info</h2>
            <p className="text-sm text-ink-400 mb-4">Order #{selectedOrder.order_number}</p>

            <div className="space-y-4">
              <div>
                <label className="text-sm text-ink-600 mb-1.5 block">Courier Name</label>
                <select
                  value={courierName}
              ...
              <div>
                <label className="text-sm text-ink-600 mb-1.5 block">Tracking ID</label>
                <input
                  type="text"
                  value={trackingInput}

  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-serif text-ink-800">Subscribers</h1>
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-14 bg-ivory-100 rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : filtered.length > 0 ? (
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-ivory-100">
              <tr>
                <th className="text-left py-3 px-4 text-ink-700">Email</th>
                <th className="text-left py-3 px-4 text-ink-700 hidden sm:table-cell">
                  Subscribed
                </th>
            ...
            <tbody>
              {filtered.map((sub) => (
                <tr
                  key={sub.id}
                  className="border-t border-stone-200 hover:bg-ivory-50"
                >
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <Mail size={16} className="text-ink-400 shrink-0" />
                      <span className="text-ink-800">{sub.email}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-ink-500 hidden sm:table-cell">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        className="p-2 rounded-2xl hover:bg-mink-100 text-mink-600"
                      ...
                      <button
                        ...
                        className="p-2 rounded-2xl hover:bg-red-100 text-red-500"
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-ink-400">No subscribers yet.</p>
        </div>
      )}
      ...
      {showCompose && (
        ...
        >
          ...
          <div
            className="relative bg-ivory-50 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6 animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-serif text-ink-800 flex items-center gap-2">
                {composeMode === 'broadcast' ? (
                  <>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-ink-400 uppercase tracking-wide mb-1 block">
                  To
                </label>
                {composeMode === 'broadcast' ? (
                  <div className="input-field bg-ivory-100 text-ink-500 text-sm">

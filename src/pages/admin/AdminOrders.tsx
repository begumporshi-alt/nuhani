    ...
    <div className="section-padding py-8 animate-fade-in">
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-ivory-100">
              <tr>
                <th className="text-left py-3 px-4 text-ink-700">Order</th>
                <th className="text-left py-3 px-4 text-ink-700 hidden md:table-cell">Date</th>
                <th className="text-left py-3 px-4 text-ink-700 hidden md:table-cell">Payment</th>
                <th className="text-left py-3 px-4 text-ink-700">Status</th>
                <th className="text-left py-3 px-4 text-ink-700 hidden lg:table-cell">Total</th>
                <th className="text-right py-3 px-4 text-ink-700"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((order) => (
                <tr key={order.id} className="border-t border-stone-200 hover:bg-ivory-50">
                  <td className="py-3 px-4">
                    <p className="font-medium text-ink-800">#{order.order_number}</p>
                    <p className="text-xs text-ink-400">{order.guest_phone ?? 'Registered user'}</p>
                  </td>
                  <td className="py-3 px-4 text-ink-500 hidden md:table-cell">
                    {new Date(order.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                  </td>
                  <td className="py-3 px-4 hidden md:table-cell">
                    <div className="flex flex-col gap-1">
                      <span className="text-ink-500 capitalize flex items-center gap-1">
                  ...
                  <td className="py-3 px-4 font-semibold text-ink-700 hidden lg:table-cell">{formatBDT(order.total_amount)}</td>
                  <td className="py-3 px-4 text-right">
                    <button onClick={() => setSelectedOrder(order)} className="p-1.5 rounded-2xl hover:bg-ivory-200 text-ink-600">
          ...
          {filtered.length === 0 && (
            <p className="text-center text-ink-400 py-12">No orders found.</p>
          )}
        </div>
      )}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelectedOrder(null)}>
          ...
          <div
            className="relative bg-ivory-50 rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-6 animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-serif text-ink-800">Order #{selectedOrder.order_number}</h2>
            </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-green-100 text-green-700 hover:bg-green-200 transition-colors text-sm font-medium"
                      >
                        <ShieldCheck size={14} /> Verify & Approve
                      </button>
                      <button
                        onClick={() => rejectCOD(selectedOrder.id)}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-red-100 text-red-600 hover:bg-red-200 transition-colors text-sm font-medium"
                      >
            ...
            <div className="flex justify-between text-sm mb-4 pb-4 border-b border-stone-200">
              ...
              <span className="text-ink-700 capitalize font-medium">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  {selectedOrder.order_items?.map((item) => (
                    <div key={item.id} className="flex justify-between text-sm bg-ivory-100 rounded-2xl p-3">
                      <div>
              ...
              <div className="bg-ivory-100 rounded-2xl p-4">
                <h4 className="text-sm font-medium text-ink-700 mb-2">Shipping Address</h4>
                {(() => {

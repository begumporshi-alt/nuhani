...
export default function AdminMessages() {
  ...
  return (
    <div className="section-padding py-8 animate-fade-in">
      <AdminBackLink />
      <div className="mb-8">
        <h1 className="text-3xl font-serif text-ink-800">Contact Messages</h1>
        <p className="text-sm text-ink-400 mt-1">
          {messages.length} total message{messages.length !== 1 ? 's' : ''}
          {unreadCount > 0 && (
            <span className="ml-2 inline-flex items-center gap-1 text-mink-500 font-medium">
              <span className="w-2 h-2 rounded-full bg-mink-500" /> {unreadCount} unread
            </span>
          )}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((msg) => (
            <button
              ...
              className={`card p-4 w-full text-left flex items-start gap-4 hover:shadow-sm transition-shadow ${
                !msg.is_read ? 'border-l-4 border-l-mink-400' : ''
              }`}
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-ink-600 truncate mb-0.5">
                  {msg.subject}
                </p>
                <p className="text-xs text-ink-400 truncate">{msg.message}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="card p-12 text-center">
          <Mail size={32} className="text-ink-300 mx-auto mb-3" />
      ...
      {viewing && (
        ...
        >
          ...
          <div
            className="relative bg-ivory-50 rounded-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6 animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-serif text-ink-800">Message Details</h2>
              <button
                onClick={() => setViewing(null)}
            ...
            <div className="space-y-3 mb-4">
              <div>
                <p className="text-xs text-ink-400 uppercase tracking-wide mb-0.5">From</p>
                <p className="text-ink-800 font-medium">{viewing.name}</p>
                <p className="text-sm text-ink-500">{viewing.email}</p>
              </div>
              <div>
                <p className="text-xs text-ink-400 uppercase tracking-wide mb-0.5">Subject</p>
                <p className="text-ink-800">{viewing.subject}</p>
              </div>
              <div>
                <p className="text-xs text-ink-400 uppercase tracking-wide mb-0.5">Received</p>
              ...
              <div className="border-t border-stone-200 pt-3">
                <p className="text-xs text-ink-400 uppercase tracking-wide mb-1">Message</p>

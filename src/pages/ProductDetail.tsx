    ...
    return (
      <div className="section-padding py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="aspect-square rounded-2xl bg-ivory-100 animate-pulse" />
          <div className="space-y-4">
            <div className="h-8 bg-ivory-100 rounded animate-pulse w-3/4" />
            <div className="h-6 bg-ivory-100 rounded animate-pulse w-1/4" />
            <div className="h-32 bg-ivory-100 rounded animate-pulse" />
          </div>
        </div>
      </div>
    )
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          {product.images && product.images.length > 1 && (
            <div className="flex gap-3 overflow-x-auto scrollbar-hide">
              {product.images.map((img, i) => (
                <button key={i} onClick={() => setSelectedImage(i)} className={`w-20 h-20 rounded-2xl overflow-hidden shrink-0 border-2 transition-all ${selectedImage === i ? 'border-mink-400' : 'border-transparent opacity-60 hover:opacity-100'}`}>
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="text-2xl font-semibold text-ink-700">{formatBDT(variant?.price ?? 0)}</span>
            {variant?.compare_at_price && variant.compare_at_price > variant.price && (
              <span className="text-stone-400 line-through text-lg">{formatBDT(variant.compare_at_price)}</span>
            )}
          </div>

          {product.description && <p className="text-ink-600 leading-relaxed mb-6">{(lang === 'bn' && product.description_bn) ? product.description_bn : product.description}</p>}
              ...
              <div className="flex flex-wrap gap-3">
                {product.variants.map((v) => (
                  <button key={v.id} onClick={() => setSelectedVariant(v.id)} disabled={v.stock_quantity === 0}
                    className={`min-w-[3rem] px-4 py-2.5 rounded-full border text-sm font-medium transition-all ${
                      selectedVariant === v.id ? 'border-ink-700 bg-ink-700 text-ivory-50'
          ...
          {!isOutOfStock && (
            <div className="mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 rounded-full border border-stone-300 flex items-center justify-center hover:bg-ivory-100 transition-colors"><Plus size={16} /></button>
              </div>
            </div>
          )}
            ...
            {reviews.map((review) => (
              <div key={review.id} className="card p-5">
                <div className="flex gap-0.5 mb-2">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={14} className={i < review.rating ? 'fill-champagne-400 text-champagne-400' : 'text-stone-300'} />
                  ))}
                </div>
                {review.title && <h4 className="font-medium text-ink-800 mb-1">{review.title}</h4>}
                {review.body && <p className="text-ink-500 text-sm leading-relaxed">{review.body}</p>}
              </div>
            ))}
          </div>
        ) : (
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              {shareLinks.map((s) => (
                <a
                  ...
                  className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-ivory-100 hover:bg-ivory-200 transition-colors"
                >
                  <div className={`w-11 h-11 rounded-full ${s.color} flex items-center justify-center text-white`}>
                    <s.icon size={20} />
                  </div>
                  <span className="text-xs text-ink-600 font-medium">{s.name}</span>
                </a>
              ))}
            </div>

            {/* Copy link */}
            <div className="flex items-center gap-2 bg-ivory-100 rounded-2xl p-3">
              <Link2 size={16} className="text-ink-400 shrink-0 ml-1" />
              <input
                type="text"
                value={shareUrl}
                readOnly
                className="flex-1 bg-transparent text-sm text-ink-600 outline-none truncate"
              />
              <button
                onClick={copyLink}
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button key={n} onClick={() => setReviewForm({ ...reviewForm, rating: n })}>
                      <Star size={28} className={n <= reviewForm.rating ? 'fill-champagne-400 text-champagne-400' : 'text-stone-300'} />
                    </button>
                  ))}
                </div>
              </div>

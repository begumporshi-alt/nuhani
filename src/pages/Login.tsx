    ...
    return (
      <div className="section-padding py-12 max-w-md mx-auto animate-fade-in">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-serif text-ink-800 mb-2">Set New Password</h1>
        ...
        {passwordUpdated ? (
          <div className="card p-6 text-center">
            ...
            <h2 className="text-xl font-serif text-ink-800 mb-2">Password Updated</h2>
            <p className="text-ink-500 text-sm mb-6">Your password has been changed successfully. You can now sign in with your new password.</p>
            <button
              onClick={() => {
          ...
          <form onSubmit={handleUpdatePassword} className="card p-6 space-y-4">
            <div>
              <label className="text-sm text-ink-600 mb-1.5 block">New Password</label>
              ...
              <p className="text-xs text-ink-400 mt-1.5">Minimum 6 characters</p>
            </div>
            <button
              type="submit"
      ...
      {resetMode ? (
        resetSent ? (
          <div className="card p-6 text-center">
            ...
            <h2 className="text-xl font-serif text-ink-800 mb-2">Check Your Email</h2>
            <p className="text-ink-500 text-sm mb-6">We've sent a password reset link to <strong>{email}</strong>. Follow the link in the email to reset your password.</p>
          ...
          <form onSubmit={handleReset} className="card p-6 space-y-4">
            ...
            <button type="submit" disabled={resetting} className="btn-primary w-full flex items-center justify-center gap-2">
              {resetting ? <div className="w-5 h-5 border-2 border-ivory-50 border-t-transparent rounded-full animate-spin" /> : 'Send Reset Link'}
            </button>
            <button type="button" onClick={() => setResetMode(false)} className="text-sm text-ink-500 hover:text-mink-500 w-full text-center">
              Back to Login
            </button>
          </form>
        )
        ...
        <>
          <form onSubmit={handleSubmit} className="card p-6 space-y-4">
            ...
            <div className="text-right">
              <button type="button" onClick={() => setResetMode(true)} className="text-sm text-mink-500 hover:text-mink-600">
                Forgot password?
              </button>
            </div>
            <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
              {loading ? <div className="w-5 h-5 border-2 border-ivory-50 border-t-transparent rounded-full animate-spin" /> : <><User size={18} /> Sign In</>}
            </button>
          </form>

          <p className="text-center text-sm text-ink-500 mt-6">
            Don't have an account?{' '}
            <Link to="/signup" className="text-mink-500 hover:text-mink-600 font-medium">Sign up</Link>
          </p>
        </>
      )}
    </div>

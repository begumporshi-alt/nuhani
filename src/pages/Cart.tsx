            ...
            return (
              <div key={(item as { id?: string }).id ?? item.variant_id} className="card p-4 flex gap-4">
                <Link to={`/product/${product?.slug ?? ''}`} className="shrink-0">
                  <img src={image} alt={product?.name ?? ''} className="w-24 h-24 rounded-2xl object-cover" />
                </Link>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="w-8 h-8 rounded-2xl border border-stone-300 flex items-center justify-center hover:bg-ivory-100 transition-colors"
                      >
                        <Minus size={14} />
                      </button>
